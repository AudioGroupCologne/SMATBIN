...
%
% Script plots_leftEarSignal_classroom
% ------------------------------------
%
% Script to generate plots of left ear signals (BRIRs/BRTFs) for the complex 
% sound field example described in [1] comparing the SMATBIN filter approach 
% with the virtual loudspeaker approach
% 
% Dependencies:
% -------------
...
N_grid = [1,7,35];               % Define 3 spatial orders to be calculated and plotted

% Load corresponding array impulse responses
% (The file size of the datasets for N = 35 is too large for GitHub. The 
% demo implementation shows the same processing for the same space with N = 5. 
% If you are interested in any of the datasets used here, please just write
% to one of the authors)
sofaObj(1) = SOFAload('src/DRIR_Classroom_LSC_SMA_P1_lbdv6_spatres.sofa');
sofaObj(2) = SOFAload('src/DRIR_Classroom_LSC_SMA_P1_lbdv86_spatres.sofa');
sofaObj(3) = SOFAload('src/DRIR_Classroom_LSC_SMA_P1_lbdv1730_spatres.sofa');

% Get SRIRs
for k = 1:length(N_grid)
    SRIR_td{k} = squeeze(sofaObj(k).Data.IR);
end
% Length of SRIRs
NFFTsrir = size(SRIR_td{1},2);

% Get Lebedev sampling grid for SMAs
for k = 1:length(N_grid)
...
%For N = 1,7,35 and above defined array configuration, load pre-calculated
%filter sets
load('src/smatbin_N_1_7_35_l');
load('src/smatbin_N_1_7_35_r');
load('src/info_N_1_7_35');

%Internal processing NFFT required for further sample-based comparison of
%SMATBIN and SOFiA
NFFT_proc       = 2*((info{1}.filter_proc_length+NFFTsrir));

%% Transform SRIRs to frequency domain

for k = 1:length(N_grid)
    SRIR_fd{k} = fft(SRIR_td{k}, NFFT_proc, 2);
    SRIR_fd{k} = SRIR_fd{k}(:, 1:round(end/2)+1);
end
...
for k = 1:length(N_grid)
    %SH transform
    SRIR_nm{k} = sofia_stc(N_grid(k), SRIR_fd{k}, grid_data_sma{k});
    
    %Binaural decoding using sofia_binauralX (virtual loudspeaker approach)
    [BRTF_sofia_l{k}, BRTF_sofia_r{k}] = sofia_binauralX(SRIR_nm{k}, dn_pw{k}, head_orientation(1), 1, sofia_hp, 2048, ncGap);
...
for k = 1:length(N_grid)
    ...
    SRIR_TF{k} = fft(SRIR_td{k}, NFFTconv, 2);
    SRIR_TF{k} = SRIR_TF{k}(:, 1:round(size(SRIR_TF{k},2))/2+1);
    ...
    for chIdx = 1:size(smatbin_l_TF{k},1)
        BRTF_smatbin_l{k} = BRTF_smatbin_l{k} + smatbin_l_TF{k}(chIdx,:) .* SRIR_TF{k}(chIdx,:);
        BRTF_smatbin_r{k} = BRTF_smatbin_r{k} + smatbin_r_TF{k}(chIdx,:) .* SRIR_TF{k}(chIdx,:);
    end
...
suppMaterial = false;
irPlotWindow = [20 3020];
NFFTplot = size(BRIR_smatbin_l{1},2);
applySmoothing = true;
nOctSM = 6;
...
for k = 1:length(N_grid)
    ...
    BRIR_sofia_l_Spec{k} = abs(fft(BRIR_sofia_l{k}, NFFTplot, 2));
    BRIR_sofia_l_Spec{k} = BRIR_sofia_l_Spec{k}(:, 1:end/2+1);
    BRIR_sofia_r_Spec{k} = abs(fft(BRIR_sofia_r{k}, NFFTplot, 2));
    BRIR_sofia_r_Spec{k} = BRIR_sofia_r_Spec{k}(:, 1:end/2+1);
    
    BRIR_smatbin_l_Spec{k} = abs(fft(BRIR_smatbin_l{k}, NFFTplot, 2));
    BRIR_smatbin_l_Spec{k} = BRIR_smatbin_l_Spec{k}(:, 1:end/2+1);
    BRIR_smatbin_r_Spec{k} = abs(fft(BRIR_smatbin_r{k}, NFFTplot, 2));
...
if applySmoothing
    for k = 1:length(N_grid)
        BRIR_sofia_l_Spec{k} = AKfractOctSmooth(BRIR_sofia_l_Spec{k},'amp',fs,nOctSM);
        BRIR_sofia_r_Spec{k} = AKfractOctSmooth(BRIR_sofia_r_Spec{k},'amp',fs,nOctSM);
        BRIR_smatbin_l_Spec{k} = AKfractOctSmooth(BRIR_smatbin_l_Spec{k},'amp',fs,nOctSM);
        BRIR_smatbin_r_Spec{k} = AKfractOctSmooth(BRIR_smatbin_r_Spec{k},'amp',fs,nOctSM);
    end 
end
...
if ~suppMaterial
    ...
    for k = 1:length(N_grid)
        ...
        text(irPlotWindow(2)-50,-0.8,['N = ',num2str(N_grid(k))],'FontSize',fontSize,'HorizontalAlignment', 'right');
        if ctn == 1
            set(gca,'yticklabel',{'\pm1','0','1'});
            legend([p1,p2],'SOFiA','SMATBIN','Location','NorthEast');
        else
    ...
    %Save as PDF
    fileName = ['SMATBINvsSOFiA_LeftEar_HeadAz',num2str(round(rad2deg(head_orientation(1)))),'Col',num2str(round(rad2deg(head_orientation(2)))),'_Classroom_LSC_SMA_N_',num2str(N_grid),'_',num2str(smatbin_filter_length),'.pdf'];
    saveas(gcf,fileName);
end
...
if suppMaterial
    ...
    for k = 1:length(N_grid)
        ...
        if ctn == 1
            set(gca,'yticklabel',{'\pm1','0','1'});
            text(irPlotWindow(1)+(110*30),1.4,['| HO = (',num2str(round(rad2deg(head_orientation(1)))),'°,',num2str(round(rad2deg(head_orientation(2)))),'°) | K = ',num2str(smatbin_filter_length),' taps |'],'FontWeight','bold','FontSize',fontSize+2,'HorizontalAlignment', 'center');
        else
            set(gca,'yticklabel',{'\pm1','0',''});
        end
        if ctn ~= 5
        ...
        if ctn == 6
            ...
            set(gca,'xticklabel',{'10','100','1k','10k'});
            set(gca,'yticklabel',{'-30','-15','0','15',''});
            legend([p2,p3_legend,p1],'SOFiA','SMATBIN','Difference','Location','South');
        end
    end
    ...
    %Save as PDF
    fileName = ['SMATBINvsSOFiA_LeftEar_HeadAz',num2str(round(rad2deg(head_orientation(1)))),'Col',num2str(round(rad2deg(head_orientation(2)))),'__Classroom_LSC_SMA_N_',num2str(N_grid),'_',num2str(smatbin_filter_length),'_SM.pdf'];
    saveas(gcf,fileName);
end