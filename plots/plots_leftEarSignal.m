...
if ~suppMaterial
    ...
    for k = 1:length(N_grid)
        ...
        ctn = ctn+1;
        %IRs
        h{ctn} = subtightplot(3,2,ctn,margin1,margin2,margin3);
        p1 = plot(abs(BRIR_sofia_l{k})-abs(BRIR_smatbin_l{k}),'LineStyle','-','LineWidth',lineWidthTop,'Color',gray);
        hold on;
        p3 = plot(abs(BRIR_sofia_l{k})*-1,'LineStyle','-','LineWidth',lineWidthTop,'Color',red);
        ...
        text(irPlotWindow(1)+2,0.8,['N = ',num2str(N_grid(k))],'FontSize',fontSize,'HorizontalAlignment', 'left');
        if ctn == 1
